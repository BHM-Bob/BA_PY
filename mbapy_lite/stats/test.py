...
LastEditTime: 2024-11-30 20:51:41
Description: 
'''
...
def mannwhitneyu(x1 = None, x2 = None,
    ...
    此包装函数同时支持直接输入和mbapy_lite-style数据输入
    """
    x1, x2 = _get_x1_x2(x1, x2, factors, tag, df)
...
def auto_ind_test(x1 = None, x2 = None,
    ...
    Params:
        ...
        - factors, tag, df: mbapy_lite-style data input
    ...
    print('-'*22, 'mbapy_lite.stats.auto_ind_test', '-'*23)
    if factors is not None and tag is not None and df is not None:
...
def chi2_contingency(observed = None,
    ...
    支持直接输入和mbapy_lite-style数据输入\n
    mbapy_lite-style: factors={'a':['a1', 'a2', ...], 'b':['b1', 'b2', ...]}, tag is value of 0/1 or number
    """
...
def f_oneway(Xs:list = None,
    ...
    2. 每个样本中的观测值都是正态分布。
    3. 每个样本中的观测值具有相同的方差。\n
    支持直接输入(Xs)和mbapy_lite-style数据输入
    """
